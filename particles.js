...
let particles = {
    ...
    mountParticles:function(){
        for(let i = 0; i < this.maxParticles; i++){
            let initialX = Math.random()*this.width;
            let initialY = Math.random()*this.height;
            let speedX = Math.random()*2*Math.pow(-1, Math.floor(Math.random()*10));
            let speedY = Math.random()*2*Math.pow(-1, Math.floor(Math.random()*10));
...
Particle.prototype.draw = function(){
    ...
    let distanceToMouse = Math.sqrt(Math.pow(this.mouseX-this.x, 2) + Math.pow(this.mouseY-this.y, 2));
